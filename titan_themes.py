...
def generate_modern_css(theme_name, h_font, b_font, hero_align, h_color, b_color, h1_size, p_size, cta_bg, cta_txt):
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .modern-hero-grid {{ display: grid; grid-template-columns: var(--h-grid-cols); gap: 4rem; align-items: center; width: 100%; }}
    .modern-hero-text {{ text-align: var(--h-text-align); display: flex; flex-direction: column; align-items: var(--h-flex-align); }}
    .hero-btn-group {{ display: flex; gap: 1rem; flex-wrap: wrap; justify-content: var(--h-btn-justify); }}
    .modern-hero-visual {{ margin: var(--h-visual-margin); max-width: 100%; display: flex; justify-content: center; }}
    ...
    .carousel-slide {{ position: absolute; inset: 0; background-size: cover; background-position: center; opacity: 0; transition: 1.5s; }}
    .carousel-slide.active {{ opacity: 1; }}

    /* GRIDS & CONTAINERS */
    .container {{ max-width: 1300px; margin: 0 auto; padding: 0 2rem; }}
    .grid-3, .modern-grid-3 {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 2.5rem; }}
    ...
    .prod-img {{ width: 100%; height: 260px; object-fit: cover; }}

    /* BUTTONS */
    .btn {{ display: inline-flex; align-items: center; justify-content: center; padding: 1.2rem 2.5rem; border-radius: var(--radius); font-weight: 800; text-decoration: none; transition: 0.3s; text-transform: uppercase; border: none; font-size: 0.95rem; letter-spacing: 1.5px; }}
    ...
    .nav-links a {{ text-decoration: none; font-weight: 600; color: var(--txt-h); }}
    ...
    /* ENHANCED UI CLASSES */
    .blog-badge {{ display: inline-block; padding: 0.4rem 1rem; background: rgba(128,128,128,0.08); color: var(--p); border-radius: 50px; font-size: 0.75rem; font-weight: 800; text-transform: uppercase; margin-bottom: 1rem; border: 1px solid rgba(128,128,128,0.15); }}
    .testimonial-card {{ padding: 2.5rem; text-align: left; }}
    .author-avatar {{ width: 45px; height: 45px; border-radius: 50%; background: var(--p); color: #fff; display: flex; align-items: center; justify-content: center; font-weight: 800; }}

    @media (max-width: 992px) {{
        .modern-hero-grid, .contact-grid {{ grid-template-columns: 1fr; text-align: center; }}
