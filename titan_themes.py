...
def generate_modern_css(theme_name, h_font, b_font, hero_align, h_color, b_color, h1_size, p_size, cta_bg, cta_txt):
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .stat-block p {{ font-weight: 700; text-transform: uppercase; letter-spacing: 1px; margin: 0; font-size: 0.9rem; color: var(--txt-b); opacity: 0.8; }}
    .stat-divider {{ width: 2px; height: 60px; background: rgba(128,128,128,0.2); }}

    /* BENTO FEATURES GRID */
    .section-subtitle {{ font-size: 1.2rem; color: var(--txt-b); opacity: 0.7; margin-top: -1rem; text-transform: uppercase; letter-spacing: 2px; font-weight: 600; text-align: center; margin-bottom: 3rem; }}
    .modern-grid-3 {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap: 2rem; }}
    .modern-feature-card {{ background: var(--card); padding: 3rem; border-radius: 24px; box-shadow: var(--shadow); border: var(--border); transition: 0.4s; display: flex; flex-direction: column; gap: 1.5rem; color: var(--txt-b); {backdrop} }}
    ...
    .card-desc {{ font-size: 0.95rem; line-height: 1.6; opacity: 0.7; margin-bottom: 1.5rem; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }}
    ...
    .thumb {{ width: 80px; height: 80px; border-radius: var(--radius); object-fit: cover; cursor: pointer; opacity: 0.6; transition: 0.3s; border: 2px solid transparent; }}
    ...
    .nav-links a {{ text-decoration: none; font-weight: 600; color: var(--txt-b); font-size: 0.95rem; transition: 0.2s; position: relative; }}
    ...
    .mobile-menu {{ display: none; font-size: 1.8rem; cursor: pointer; background:none; border:none; color:var(--txt-b); }}
    ...
    #cart-modal h3, #lang-modal h3 {{ margin-bottom: 1.5rem; color: var(--p); font-size: 1.8rem; border-bottom: 1px solid rgba(128,128,128,0.1); padding-bottom: 1rem; }}
    ...
    /* FLOATING WIDGETS (WA, Voice, Theme, Cart) */
    ...
    @media (max-width: 992px) {{
        .modern-hero {{ padding-top: 100px; text-align: center; }}
        .modern-hero-grid {{ grid-template-columns: 1fr; gap: 2rem; }}
        .modern-hero-text {{ display: flex; flex-direction: column; align-items: center; }}
        .visual-frame {{ height: 350px; }}
        .stats-ribbon {{ flex-direction: column; gap: 2rem; padding: 2rem; }}
