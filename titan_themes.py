...
THEME_REGISTRY = {
    ...
    "19. Architecture Grid": {"bg": "#ffffff", "txt": "#000000", "card": "#f4f4f5", "p": "#000000", "s": "#3b82f6", "nav": "rgba(255,255,255,0.9)", "shadow": "none", "radius": "0px", "border": "1px solid #000000"},
    ...
    "22. Monochromatic Black/White": {"bg": "#ffffff", "txt": "#000000", "card": "#ffffff", "p": "#000000", "s": "#000000", "nav": "rgba(255,255,255,0.9)", "shadow": "4px 4px 0px #000000", "radius": "0px", "border": "2px solid #000000"},
    ...
    "24. Gradient Mesh": {"bg": "linear-gradient(45deg, #ff9a9e 0%, #fecfef 99%, #fecfef 100%)", "txt": "#333333", "card": "rgba(255,255,255,0.6)", "p": "#f77062", "s": "#3f51b5", "nav": "rgba(255,255,255,0.6)", "shadow": "0 8px 32px rgba(0,0,0,0.1)", "radius": "30px", "border": "1px solid rgba(255,255,255,0.5)"},
...
def generate_modern_css(theme_name, h_font, b_font, hero_align, anim_type, overlay_opacity):
    ...
    if anim_type == "Fade Up":
        anim_css = ".reveal { opacity: 0; transform: translateY(40px); transition: all 0.8s cubic-bezier(0.25, 0.46, 0.45, 0.94); } .reveal.active { opacity: 1; transform: translateY(0); }"
        ...
        anim_css = ".reveal { opacity: 0; transform: translateX(-40px); transition: all 0.8s ease-out; } .reveal.active { opacity: 1; transform: translateX(0); }"
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    body {{ background: var(--bg); color: var(--txt); font-family: var(--b-font); line-height: 1.6; overflow-x: hidden; transition: background 0.4s ease; -webkit-font-smoothing: antialiased; }}
    ...
    .hero-overlay {{ background: linear-gradient(to bottom, rgba(0,0,0,{overlay_opacity - 0.2}) 0%, rgba(0,0,0,{overlay_opacity + 0.2}) 100%); position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: 1; }}
    ...
    .card {{ background: var(--card); border-radius: var(--radius); border: var(--border); box-shadow: var(--shadow); transition: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); display: flex; flex-direction: column; overflow: hidden; position: relative; {backdrop} }}
    ...
    .detail-view {{ display: grid; grid-template-columns: 1.2fr 1fr; gap: 5rem; align-items: start; background: var(--card); padding: 4rem; border-radius: var(--radius); box-shadow: var(--shadow); border: var(--border); {backdrop} }}
    .product-price-tag {{ font-size: 3rem; color: #059669; font-weight: 900; margin-bottom: 2rem; display: block; }}
    ...
    .blog-badge {{ background: var(--s); color: #fff; padding: 0.3rem 0.8rem; border-radius: 50px; font-size: 0.75rem; text-transform: uppercase; font-weight: bold; width: fit-content; margin-bottom: 1rem; display:inline-block; }}
    #cart-float {{ position: fixed; bottom: 100px; right: 30px; background: var(--p); color: #fff; padding: 15px 20px; border-radius: 50px; box-shadow: 0 10px 20px rgba(0,0,0,0.2); cursor: pointer; z-index: 998; display: flex; align-items: center; gap: 10px; font-weight: bold; }}
    ...
    #voice-btn {{ position: fixed; bottom: 170px; right: 30px; background: var(--p); color: #fff; border-radius: 50px; width: 50px; height: 50px; display: flex; align-items: center; justify-content: center; font-size: 1.2rem; cursor: pointer; box-shadow: 0 10px 20px rgba(0,0,0,0.2); z-index: 998; border: none; }}
    ...
    #top-bar {{ position: fixed; top: 0; width: 100%; background: var(--s); color: #fff; text-align: center; padding: 10px; z-index: 1002; font-weight: bold; font-size: 0.9rem; transition: transform 0.3s; }}
    #top-bar a {{ color: #fff; text-decoration: underline; }}
    #lead-popup {{ display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: var(--card); padding: 3rem; text-align: center; border-radius: var(--radius); z-index: 2000; box-shadow: 0 25px 100px rgba(0,0,0,0.5); width: 90%; max-width: 450px; border: var(--border); color: var(--txt); }}
    ...
    @media (max-width: 992px) {{
        nav#main-navbar .nav-links {{ position: fixed; top: 70px; left: -100%; width: 100%; height: calc(100vh - 70px); background: var(--bg); flex-direction: column; padding: 3rem; transition: 0.4s ease; align-items: center; justify-content: center; gap: 2.5rem; overflow-y:auto; }}
        nav#main-navbar .nav-links.active {{ left: 0; }}
        .nav-links a {{ font-size: 1.5rem; }}
        .mobile-menu {{ display: block; }}
        .about-grid, .detail-view, .contact-grid, .grid-3 {{ grid-template-columns: 1fr !important; gap: 3rem; }}
