...
def generate_modern_css(theme_name, h_font, b_font, hero_align, h_color, b_color, h1_size, p_size, cta_bg, cta_txt):
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    main section {{ padding: clamp(2rem, 8vw, 8rem) 0; position: relative; z-index: 5; }}
    .section-head {{ text-align: center; margin-bottom: clamp(3rem, 5vw, 5rem); }}
    .grid-3 {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 2.5rem; align-items: stretch; }}
    ...
    .detail-view {{ 
        display: grid; grid-template-columns: 0.8fr 1.2fr; gap: 6rem; align-items: start; 
        background: var(--card); padding: 5rem; border-radius: 32px; box-shadow: var(--shadow); 
    ...
    .product-info-column h1 {{ font-size: clamp(1.8rem, 3vw, 2.5rem) !important; margin-bottom: 1rem !important; line-height: 1; }}
    .product-specs-container, .product-specs-container p {{ font-size: 1rem !important; line-height: 1.6 !important; opacity: 0.8 !important; color: var(--txt-b); }}
    ...
    /* SMART OVERLAYS (Cart, Popup, Lang) */
    ...
    #cart-overlay, #lang-overlay {{ display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.7); backdrop-filter: blur(5px); z-index: 3000; }}
    #cart-modal, #lang-modal, #lead-popup {{ display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: var(--card); width: 90%; max-width: 500px; max-height: 90vh; overflow-y: auto; padding: 2.5rem; border-radius: 24px; box-shadow: 0 30px 60px rgba(0,0,0,0.4); z-index: 3001; border: var(--border); color: var(--txt-b); }}
    ...
    .lang-grid {{ display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-top: 1rem; }}
    .lang-opt {{ padding: 1.2rem; border: var(--border); border-radius: 12px; cursor: pointer; font-weight: 700; transition: 0.3s; background: var(--bg); text-align: center; }}
    .lang-opt:hover {{ background: var(--p); color: #fff; }}

    /* FLOATERS (Top bar, Theme Toggle, Voice) */
    #top-bar {{ position: fixed; top: 0; left: 0; width: 100%; background: var(--s); color: #fff; text-align: center; padding: 12px; z-index: 2005; font-weight: 800; font-size: 0.95rem; box-shadow: 0 4px 10px rgba(0,0,0,0.1); }}
    ...
    #voice-btn {{ position: fixed; bottom: 170px; right: 30px; background: var(--p); color: #fff; border-radius: 50%; width: 50px; height: 50px; display: flex; align-items: center; justify-content: center; font-size: 1.2rem; cursor: pointer; box-shadow: 0 10px 25px rgba(0,0,0,0.2); z-index: 1000; transition: 0.3s; }}
    ...
    /* ========================================================================= */
    /* --- NEW REPAIRED BENTO GRID SYSTEM --- */
    /* ========================================================================= */
    ...
    .bento-card, .modern-feature-card {{
        ...
        display: flex;
        flex-direction: column;
        height: 100%; /* Stretches beautifully to fill the grid cell */
        position: relative;
        overflow: hidden;
    ...
    .bento-card:hover, .modern-feature-card:hover {{
    ...
    .feature-content div, .feature-content p {{
        margin: 0;
        line-height: 1.6;
        color: var(--txt-b);
    }}
    ...
    .asym-badge {{
        position: absolute; bottom: 30px; right: 30px; background: var(--s); color: #fff !important;
        padding: 1rem 1.5rem; font-weight: 800; font-size: 1.1rem; border-radius: 16px;
        z-index: 6; box-shadow: 0 10px 20px rgba(0,0,0,0.2);
    }}
    .tagline {{ color: var(--p); font-weight: 800; text-transform: uppercase; letter-spacing: 2px; margin-bottom: 1rem; font-size: 0.9rem; }}


    /* ========================================================================= */
    /* --- MOBILE OPTIMIZATION (TABLET / RESPONSIVE GRIDS) --- */
    /* ========================================================================= */
    ...
    /* ========================================================================= */
    /* --- MOBILE OPTIMIZATION (PHONES / OVERLAY FIXES) --- */
    /* ========================================================================= */
