...
def generate_modern_css(theme_name, h_font, b_font, hero_align, h_color, b_color, h1_size, p_size, cta_bg, cta_txt):
    ...
    gradient_text = f"background: linear-gradient(90deg, {t['p']}, {t['s']}); -webkit-background-clip: text; -webkit-text-fill-color: transparent;" if any(x in theme_name for x in ["SaaS", "Dark", "Creative"]) else ""
    btn_hover = "transform: translateY(-3px) scale(1.02); filter: brightness(1.15); box-shadow: 0 10px 25px -5px var(--p);"
    if any(x in theme_name for x in ["Brutalist", "Cyberpunk", "Monochromatic"]): btn_hover = "transform: translate(-4px, -4px); box-shadow: 8px 8px 0px #000;"
    ...
    if hero_align == "Center":
        h_align, grid_cols, flex_align, txt_align, p_margin = "text-align: center; justify-content: center;", "1fr", "center", "center", "0 auto 1.5rem auto"
    else:
        h_align, grid_cols, flex_align, txt_align, p_margin = "text-align: left; justify-content: flex-start; align-items: center;", "1.1fr 1fr", "flex-start", "left", "0 0 1.5rem 0"
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    body {{ background: var(--bg); color: var(--txt-b); font-family: var(--b-font); font-size: var(--p-size); line-height: 1.6; overflow-x: hidden; width: 100%; }}
    ...
    /* 2026 HERO ENGINE */
    .hero {{ position: relative; min-height: 95vh; display: flex; {h_align} padding-top: 120px; }}
    .carousel-slide {{ position: absolute; inset: 0; background-size: cover; background-position: center; opacity: 0; transition: opacity 1.5s; z-index: 0; }}
    .carousel-slide.active {{ opacity: 1; }}
    .hero-content {{ z-index: 2; position: relative; width: 100%; padding: 0 5%; max-width: 1400px; }}
    .hero h1 {{ color: #ffffff !important; text-shadow: 0 10px 30px rgba(0,0,0,0.5); }}

    /* MODERN NAVBAR */
    nav#main-navbar {{ position: fixed; top: 0; width: 100%; z-index: 2000; background: var(--nav); backdrop-filter: blur(20px); border-bottom: 1px solid rgba(128,128,128,0.1); padding: 1.2rem 0; }}
    .nav-flex {{ display: flex; justify-content: space-between; align-items: center; max-width: 1300px; margin: 0 auto; padding: 0 2rem; }}
    .nav-links {{ display: flex; align-items: center; gap: 2rem; }}
    .nav-links a {{ text-decoration: none; color: var(--txt-h); font-weight: 600; font-size: 0.95rem; }}
    .mobile-menu {{ display: none; font-size: 1.8rem; cursor: pointer; color: var(--txt-h); }}

    /* UTILS */
    .container {{ max-width: 1300px; margin: 0 auto; padding: 0 2rem; }}
    main section {{ padding: 6rem 0; position: relative; }}
    .section-head {{ text-align: center; margin-bottom: 4rem; }}
    .grid-3 {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 2.5rem; }}

    /* BUTTONS */
    .btn {{ display: inline-flex; align-items: center; justify-content: center; padding: 1.2rem 2.5rem; border-radius: var(--radius); font-weight: 800; text-decoration: none; transition: 0.3s; text-transform: uppercase; cursor: pointer; border: none; font-size: 0.9rem; letter-spacing: 1px; }}
    .btn-primary {{ background: var(--p); color: #fff !important; }}
    .btn-accent {{ background: var(--s); color: #fff !important; }}
    .btn:hover {{ {btn_hover} }}

    /* ==========================================================
       BENTO & FEATURES SECTION (GAP-FREE ARCHITECTURE)
       ========================================================== */
    #features .container > div:last-child {{
        display: grid !important;
        grid-template-columns: repeat(3, 1fr) !important;
        gap: 1.5rem !important;
        align-items: flex-start !important; /* LOCKS ALL CARDS TO TOP */
    }}
    ...
    .modern-feature-card, .bento-card {{
        ...
        height: auto !important; /* SEALED: No empty stretching */
    ...
    #features .container > div:last-child > div:first-child {{
        grid-column: span 2 !important; /* Wide Hero Card */
    }}
    ...
    .asym-image {{ grid-column: 5 / 13; grid-row: 1; z-index: 1; height: 550px; border-radius: 32px; overflow: hidden; position: relative; }}
    .asym-image img {{ width: 100%; height: 100%; object-fit: cover; }}
    .asym-badge {{ position: absolute; bottom: 30px; right: 30px; background: var(--s); color: #fff !important; padding: 1rem 1.5rem; font-weight: 800; border-radius: 16px; z-index: 6; }}

    /* STATS RIBBON */
    .stats-ribbon {{ background: var(--card); border-radius: 24px; padding: 3rem; display: flex; justify-content: space-around; align-items: center; box-shadow: var(--shadow); border: var(--border); margin-top: -60px; position: relative; z-index: 100; }}
    .stat-block h3 {{ font-size: 3.5rem; color: var(--p); line-height: 1; }}

    /* WIDGETS */
    #wa-widget {{ position: fixed; bottom: 30px; right: 30px; background: #25D366; width: 60px; height: 60px; border-radius: 50%; display: flex; align-items: center; justify-content: center; z-index: 1000; box-shadow: 0 10px 25px rgba(0,0,0,0.2); }}
    #theme-toggle {{ position: fixed; bottom: 30px; left: 30px; width: 50px; height: 50px; background: var(--card); border-radius: 50%; display: flex; align-items: center; justify-content: center; z-index: 1000; border: var(--border); cursor: pointer; }}

    /* FOOTER */
    footer {{ background: #0f172a; color: #f8fafc; padding: 6rem 0 3rem 0; border-top: 4px solid var(--p); }}
    ...
    @media (max-width: 992px) {{
        ...
        .asym-container {{ display: flex; flex-direction: column-reverse; }}
        .asym-text-box {{ width: 90%; margin: -80px auto 0; border-left: none; border-top: 8px solid var(--p); }}
        .asym-image {{ width: 100%; height: 350px; }}
        #features .container > div:last-child {{ grid-template-columns: 1fr !important; }}
        #features .container > div:last-child > div:first-child {{ grid-column: span 1 !important; }}
