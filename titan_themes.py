# titan_themes.py
# 2026 Titan Architecture Theme Engine - V5.0 MEGA EDITION
# Features: 25 Themes, Asymmetrical Layouts, Sticky Media, Typography Mastery
...
def generate_modern_css(theme_name, h_font, b_font, hero_align, h_color, b_color, h1_size, p_size, cta_bg, cta_txt):
    ...
    gradient_text = ""
    if any(keyword in theme_name for keyword in ["SaaS", "Dark", "Creative", "Cyberpunk", "Synthwave"]):
    ...
    if any(k in theme_name for k in ["Brutalist", "Cyberpunk", "Monochromatic"]):
        btn_hover = "transform: translate(-4px, -4px); box-shadow: 8px 8px 0px #000;"

    backdrop = "backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px);" if any(k in theme_name for k in ["Glass", "Mesh", "SaaS"]) else ""
