...
def gen_csv_parser():
    ...
    function parseMarkdown(text) { 
        if (!text) return '';
        // Fix the literal \\n string by splitting and joining with <br>
        ...
        return bolded.split('<br>').map(line => {
            let t = line.trim();
            if (t.startsWith('* ')) {
                return '<li style="margin-left:20px; list-style-type:disc; margin-bottom:8px;">' + t.substring(2) + '</li>';
            }
...
def gen_footer():
    icons = ""
    # We wrap each icon to ensure it respects the .social-icon class
    ...
    if x_link: icons += f'<a href="{x_link}" target="_blank" aria-label="X"><svg class="social-icon" viewBox="0 0 24 24"><path d="M18.901 1.153h3.68l-8.04 9.19L24 22.846h-7.406l-5.8-7.584l-6.638 7.584H.474l8.6-9.83L0 1.154h7.594l5.243 6.932ZM17.61 20.644h2.039L6.486 3.24H4.298Z"></path></svg></a>'
    ...
    if yt_link: icons += f'<a href="{yt_link}" target="_blank" aria-label="YouTube"><svg class="social-icon" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.5 12 3.5 12 3.5s-7.505 0-9.377.55a3.016 3.016 0 0 0-2.122 2.136C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.55 9.376.55 9.376.55s7.505 0 9.377-.55a3.016 3.016 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg></a>'
    
    # High-contrast Service Area section
    service_area_html = f'''
    <div style="margin-top: 2rem; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 1.5rem;">
        <span style="color: #ffffff !important; font-weight: 800; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 2px; display: block; margin-bottom: 10px;">Service Area</span>
        <span style="color: rgba(255,255,255,0.9) !important; font-size: 0.95rem; line-height: 1.6; display: block;">{seo_area}</span>
    </div>
    ''' if seo_area else ""
    ...
    <div>
        ...
        <p style="color:rgba(255,255,255,0.8); font-size:0.95rem;">{biz_addr}</p>
        <div class="social-row">{icons}</div>
        {service_area_html}
    </div>
    <div>
        <h4 style="color:white; text-transform:uppercase; margin-bottom:1.5rem;">Quick Links</h4>
        <a href="index.html" style="color:#94a3b8!important; display:block; margin-bottom:0.8rem;">Home</a>
        <a href="about.html" style="color:#94a3b8!important; display:block; margin-bottom:0.8rem;">About</a>
        {f'<a href="blog.html" style="color:#94a3b8!important; display:block; margin-bottom:0.8rem;">Blog</a>' if show_blog else ""}
        {f'<a href="booking.html" style="color:#94a3b8!important; display:block; margin-bottom:0.8rem;">Book Now</a>' if show_booking else ""}
    </div>
    <div>
        <h4 style="color:white; text-transform:uppercase; margin-bottom:1.5rem;">Legal Compliance</h4>
        <a href="privacy.html" style="color:#94a3b8!important; display:block; margin-bottom:0.8rem;">Privacy Policy</a>
        <a href="terms.html" style="color:#94a3b8!important; display:block; margin-bottom:0.8rem;">Terms of Service</a>
    </div>
    </div><div style="border-top:1px solid rgba(255,255,255,0.05); margin-top:4rem; padding-top:2rem; text-align:center; color:#64748b; font-size:0.85rem;">&copy; {datetime.datetime.now().year} {biz_name}. All Rights Reserved.</div></div></footer>
    """
def gen_scripts():
...
def gen_product_page_content(is_demo=False):
    ...
    async function loadProduct() {{
        ...
        if(isDemo && !targetName) targetName = "The Blueprint Authority";
        ...
        try {{
            ...
            for(let i=1; i<lines.length; i++) {{
                ...
                if(clean[0] === targetName || (isDemo && i===1)) {{
                    ...
                    let thumbHtml = '';
                    allImgs.forEach(img => {{ thumbHtml += `<img src="${{img.trim()}}" class="thumb" onclick="changeImg('${{img.trim()}}')" style="width:70px; height:70px; object-fit:cover; margin-right:10px; cursor:pointer; border-radius:12px; border:1px solid rgba(128,128,128,0.2); transition:0.3s;">`; }});
                    
                    let mainMedia = `<img src="${{allImgs[0]}}" id="main-img" style="width:100%; border-radius:24px; height:500px; object-fit:cover; box-shadow: var(--shadow);" alt="${{clean[0]}}">`;
                    ...
                    document.getElementById('product-detail-target').innerHTML = `
                        <div class="detail-view">
                            ...
                            <div class="product-info-column">
                                ...
                                <div class="product-specs-container" style="margin-top:2rem;">
                                    ${{parseMarkdown(clean[2])}}
                                </div>

                                <div style="margin-top:4rem; position: sticky; bottom: 20px; z-index: 10;">
                                    ${{btnAction}}
                                </div>
                                ...
                                <div style="margin-top:60px; padding-top:30px; border-top:1px solid rgba(128,128,128,0.1);">
                                    ...
                                    <div class="share-row" style="display:flex; gap:15px;">
                                        ...
                                        <a href="https://www.linkedin.com/shareArticle?mini=true&url=${{u}}&title=${{t}}" target="_blank" style="background:#0A66C2; width:45px; height:45px; display:flex; align-items:center; justify-content:center; border-radius:12px; color:white; text-decoration:none;"><svg viewBox="0 0 24 24" fill="white" width="20"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2a2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6zM2 9h4v12H2zM4 2a2 2 0 1 1-2 2a2 2 0 0 1 2-2z"></path></svg></a>
                                        <button onclick="navigator.clipboard.writeText(window.location.href); alert('Link Copied!')" style="background:#64748b; width:45px; height:45px; display:flex; align-items:center; justify-content:center; border-radius:12px; color:white; border:none; cursor:pointer;"><svg viewBox="0 0 24 24" fill="white" width="20"><path d="M3.9 12c0-1.71 1.39-3.1 3.1-3.1h4V7H7c-2.76 0-5 2.24-5 5s2.24 5 5 5h4v-1.9H7c-1.71 0-3.1-1.39-3.1-3.1zM8 13h8v-2H8v2zm9-6h-4v1.9h4c1.71 0 3.1 1.39 3.1 3.1s-1.39 3.1-3.1 3.1h-4V17h4c2.76 0 5-2.24 5-5s-2.24-5-5-5z"/></svg></button>
                                    </div>
                                </div>
                            </div>
                        </div>`;
